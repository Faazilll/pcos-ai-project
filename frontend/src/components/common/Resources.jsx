...
const Resources = ({ setActiveTab }) => {
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      <h2 className="text-2xl font-bold text-teal-400 text-teal-400 mb-6">Resources</h2>

      {/* Articles list */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {articles.map((a) => (
          <div key={a.id} className="aura-card cursor-pointer" onClick={() => setSelectedArticle(a)}>
            <h3 className="text-xl font-semibold text-teal-600 mb-2">{a.title}</h3>
      ...
      <div className="mt-6 flex gap-3">
        <button onClick={() => setActiveTab('doctors')} className="aura-button aura-button">Find Doctors</button>
        <button onClick={() => setActiveTab('community')} className="aura-button aura-button">Community</button>
        <button onClick={() => setActiveTab('home')} className="aura-button">Back to Home</button>
      </div>
    </div>
  );
};
