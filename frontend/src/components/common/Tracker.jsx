  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-8 px-4">
      <div className="aura-card">
        <h2 className="text-2xl font-bold text-teal-400 text-teal-400 mb-6">Period & Symptom Tracker</h2>
        
        <div className="mb-6 flex justify-center space-x-4">
          <button 
            onClick={() => setView('form')}
            className={`px-4 py-2 rounded-lg ${view === 'form' ? 'bg-teal-600 text-dark' : 'aura-card'}`}
          >
            Log Period
          </button>
          <button 
            onClick={() => setView('history')}
            className={`px-4 py-2 rounded-lg ${view === 'history' ? 'bg-sky-500 text-dark' : 'aura-card'}`}
          >
            History
          </button>
          <button 
            onClick={() => setView('calendar')}
            className={`px-4 py-2 rounded-lg ${view === 'calendar' ? 'bg-teal-400 text-dark' : 'aura-card'}`}
          >
            Predictions
          </button>
        </div>
          ...
          <form onSubmit={handleCycleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="aura-input w-full h-24"
                placeholder="Add any additional notes about your cycle or symptoms..."
              ></textarea>
            </div>

            <button type="submit" className="aura-button aura-button w-full">Save Period Data</button>
          </form>
        )}

        {view === 'history' && (
          <div className="space-y-6">
            <h3 className="text-xl text-sky-500 mb-4">Your Period History</h3>
            
            {cycles.length > 0 ? (
              <div className="space-y-4">
                {cycles.map((cycle, index) => (
                  <div key={cycle.id} className="aura-card">
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="text-teal-400 font-semibold">Period #{cycles.length - index}</h4>
                      <button 
                      ...
                      <div className="mb-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {cycle.symptoms.map(symptom => (
                            <span key={symptom} className="aura-card text-sky-500 text-xs px-2 py-1 rounded">
                              {symptom}
                            </span>
                          ))}
                        </div>
              ...
              <div className="aura-card text-center">
        ...
        {view === 'calendar' && (
          <div className="space-y-6">
            <h3 className="text-xl text-teal-400 mb-4">Period Predictions</h3>
            
            {prediction ? (
              <div className="aura-card">
                <div className="mb-6">
                  <h4 className="text-sky-500 font-semibold mb-2">Next Period Prediction</h4>
                  <div className="aura-card p-4 rounded-lg flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-gray-400 text-sm">Expected Start Date</p>
                      <p className="text-2xl font-bold text-sky-500">{formatDate(prediction.nextPeriod)}</p>
                    </div>
                  </div>
                </div>
                
                <div>
                  <h4 className="text-teal-400 font-semibold mb-2">Fertility Window</h4>
                  <div className="aura-card p-4 rounded-lg">
                    <div className="flex justify-between items-center">
                      <div className="text-center">
                        <p className="text-gray-400 text-sm">Start</p>
                        <p className="text-lg font-semibold text-teal-400">{formatDate(prediction.fertileWindowStart)}</p>
                      </div>
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-lg font-semibold text-teal-600">{formatDate(prediction.fertileWindowEnd)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 text-center">
                  <p className="text-soft-white text-sm">
                    Based on your average cycle length of <span className="text-teal-400 font-semibold">{averageCycleLength} days</span>
                  </p>
              ...
              <div className="aura-card text-center">
                ...
                {cycles.length === 0 && (
                  <button 
                    onClick={() => setView('form')}
                    className="aura-button aura-button"
                  >
                    Start Tracking
                  </button>
                )}
